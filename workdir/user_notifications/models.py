...
class Notification(models.Model):
    ...
    content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE, null=True, blank=True, related_name='target_notifications') # Added related_name
    ...
    # Changed related_name for actor_content_type to avoid clash if ContentType is same for actor and target
    actor_content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE, null=True, blank=True, related_name='actor_notifications_triggered')
...
class Activity(models.Model):
    ...
    target_content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE, null=True, blank=True, related_name='target_activities_involved_in') # Added related_name
    target_object_id = models.PositiveIntegerField(null=True, blank=True)
    target = GenericForeignKey('target_content_type', 'target_object_id')

    # Optional: The context of the action (e.g., the workspace where a page was created)
    context_content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE, null=True, blank=True, related_name='context_activities_occurred_in') # Added related_name
